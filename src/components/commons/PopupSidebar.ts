...
const createPopupSidebar = ({ 
  ...
  const calculateDimensions = (w: number, h: number): PopupDimensions => {
    ...
    const sidebarHeight = panelHeight * 0.70; // Leave space for title
    ...
    const contentHeight = sidebarHeight * 0.89;
  ...
  const closeButton = createButton({
    ...
    width: height * 0.08,
    height: height * 0.08,
    x: dimensions.panelWidth - dimensions.panelWidth * 0.08,
    y: dimensions.panelY + (height * 0.17),
  ...
  const renderSidebarButtons = () => {
    ...
    sidebarButtons.forEach((buttonConfig, index) => {
      ...
      const bgWidth = dimensions.sidebarWidth - (bgPadding * 4);
      ...
      if (buttonConfig.texture) {
        try {
          ...
          // FIXED: Normalize all textures to the same size for visual consistency
          // Define a fixed target size for all sidebar button textures
          const targetSize = Math.min(dimensions.sidebarWidth * 0.8, buttonHeight * 0.8);

          // Calculate scale to make all textures the same size (not aspect-ratio dependent)
          const scale = targetSize / dimensions.sidebarWidth;

          buttonIcon.scale.set(0.16);
          buttonIcon.anchor.set(0);
          buttonIcon.x = dimensions.sidebarWidth / 15;
          buttonIcon.y = buttonHeight / 2;

          console.log(`🔧 Icon for ${buttonConfig.label}: normalized scale=${scale}, target size=${targetSize}, final size=${buttonIcon.width}x${buttonIcon.height}`);
...
export default createPopupSidebar;