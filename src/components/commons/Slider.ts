...
function createSlider(width: number, initialValue: number, onChange: (value: number) => void): Container {
  ...
  const trackColor = 0x567086; // 
  const knobColor = 0x304553; // 
  const knobStrokeColor = 0x0E212E; // 
  const knobColorOnClick = 0x0E212E; // 
  ...
  knob.stroke({ color: knobStrokeColor, width: 3 });
  knob.y = height / 2;
  ...
  const updateKnobAppearance = (isDragging: boolean) => {
    if (isDragging) {
      ...
      knob.fill({ color: knobColorOnClick }); // Brighter fill
      knob.stroke({ color: knobStrokeColor, width: 3 }); // Brighter gold border
    } else {
